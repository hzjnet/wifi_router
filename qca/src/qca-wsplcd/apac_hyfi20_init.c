        ...
        if (freq != APAC_WIFI_FREQ_INVALID) {
            if (pAP[freq].valid) {
                if(freq == APAC_WIFI_FREQ_5 &&
                    ...
                    dprintf(MSG_MSGDUMP, " Getting radio index %d for vapIndex %d \n", radio_index, hyif->vapIndex);
                    dprintf(MSG_MSGDUMP, " Radio index %d for freq %d \n", pAP[hyif->wifiFreq].radio_index, freq);
                } else {
                    dprintf(MSG_MSGDUMP, "%s - Configuration: Freq %d has more than Two 1905 AP, previous\
