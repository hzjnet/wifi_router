        ...
        wlanif_associatedSTAsCB callback, void *cookie)
{
    ...
    {
        dbgf(state->dbgModule, DBGERR,
             "%s: getStaInfo() API call failed", __func__);
        return LBD_NOK;
    }
    ...
    for (i=0; i < getStaInfo.numSta; i++)
    {
        ...
        callback(&sta_addr, &bss, (LBD_BOOL)(getStaInfo.stainfo[i].isbtm),
                (LBD_BOOL)(getStaInfo.stainfo[i].isrrm),
                isMUMIMOSupported, isStaticSMPS, getStaInfo.stainfo[i].operating_band, &phyCapInfo, cookie);
...
#if SONLIB_SUPPORT_ENABLED
        if((vap->drvHandle->ops->setMacFiltering(vap->drvHandle->ctx,
                 vap->ifname, staAddr->ether_addr_octet,
                 ((operation==IO_OPERATION_ADDMAC)?LBD_TRUE:LBD_FALSE))) < 0) {
#else
        if((wlanIfLb->ops->addDelKickMAC(wlanIfLb->ctx, vap->ifname,
                               operation,(char *) &addr , sizeof(addr))) < 0) {
#endif
            dbgf(state->dbgModule, DBGERR,
