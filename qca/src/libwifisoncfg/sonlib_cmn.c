 ...
 */
static struct soncmnDrvHandle * soncmnInit(enum soncmnVendors vendor)
{
    ...
    switch (vendor)
    {
        ...
        case SONCMN_VENDOR_ATHXK:
            //drvhandle->ops = son_getops_athxk();
            break;
    ...
    if ( drvhandle->ops->init((drvhandle)))
    {
           ...
           const char* radioVendorMap, uint8_t isHYD, uint8_t isLBD)
{
    ...
    for (i = 0; i < numInterfaces; i++) {
        ...
        if( !vendorDrvHandleMap[vEnum] ) {
            ...
            vendorDrvHandleMap[vEnum] = soncmnInit(soncmnGetEnumForVendorName (vendor));
            ...
            dbgf(soncmnDbgS.dbgModule, DBGINFO, "%s: DrvHandle (%p) was already created for Vendor %s", __func__,
                                                                   vendorDrvHandleMap[vEnum], vendor);
            vendorDrvHandleMap[vEnum]->ops->init(vendorDrvHandleMap[vEnum]);
        }
 ...
 *
 */
static int soncmnGetIfIndex(const char *ifname)
{
    int i, index;
    index = -1;
