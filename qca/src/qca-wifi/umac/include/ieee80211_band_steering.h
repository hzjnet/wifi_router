 ...
 *
 * @param [in] vap  the VAP on which the change occurred
 * @param [in] AP cap struct to fill info.
 * @param [in] data pointer to fill operating class information.
...
static inline int wlan_bsteering_get_apcap(struct ieee80211vap *vap, void *apcap,
                             u_int8_t *data) {
    return -EINVAL;
}

static inline int wlan_bsteering_get_assoc_frame(struct ieee80211vap *vap, const u_int8_t *macaddr, char *info,
                                   u_int32_t *length) {
    return -EINVAL;
}
