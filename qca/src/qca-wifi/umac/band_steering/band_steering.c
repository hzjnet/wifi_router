...
#define MAP_SUCCESS 0x00
    ...
    if (ni->assoc_frame) {
        if (wbuf_get_pktlen(ni->assoc_frame) <= *length)
            OS_MEMCPY(info, wbuf_header(ni->assoc_frame), wbuf_get_pktlen(ni->assoc_frame));
        else {
            ieee80211_free_node(ni);
            return MAP_UNSPECIFIED_FAILURE;
        }
...
bool wlan_bsteering_get_apcap(struct ieee80211vap *vap, ieee80211req_map_apcap_data *apcap,
                              u_int8_t *data) {
    struct ieee80211_ath_channel *channelmin = NULL, *channelmax = NULL;
    u_int8_t minch, maxch;
    int8_t maxpow, minpow, pow;
    u_int8_t ht40mode, htmask = 0, vhtmask = 0, vhtmask1 = 0;
    struct ieee80211com *ic = NULL;
    ...
    ic = vap->iv_ic;
    IEEE80211_ADDR_COPY(apcap->radiomac, vap->iv_my_hwaddr);
    ...
    apcap->opclasscnt = regdmn_get_map_opclass(vap->iv_ic, minch, maxch, &data, pow);
    // keeping it to max now , should be changed later
    apcap->maxbss = CFG_MAX_VAPS;
    ...
    tx_streams = tx_streams - 1;
    rx_streams = rx_streams - 1;

    htmask = htmask | (tx_streams << 6);
    htmask = htmask | (rx_streams << 4);
    ...
    if (vap->iv_des_mode >= 9 && vap->iv_des_mode <= 14) {
        ht40mode = 1;
        htmask = htmask | (ht40mode << 1);
    }

    apcap->htcap = htmask;

    /* Add VHT cap */
    apcap->vhttxmcs = vap->iv_vhtcap_max_mcs.tx_mcs_set.mcs_map;
    apcap->vhtrxmcs = vap->iv_vhtcap_max_mcs.rx_mcs_set.mcs_map;

    vhtmask = vhtmask | (tx_streams << 5);
    vhtmask = vhtmask | (rx_streams << 2);
