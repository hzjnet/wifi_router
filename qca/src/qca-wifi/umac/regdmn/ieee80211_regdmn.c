...
uint8_t regdmn_get_map_opclass(struct ieee80211com *ic, u_int8_t minch, u_int8_t maxch,
                               u_int8_t **data, int8_t pow) {
    regdmn_op_class_map_t *map;
    uint8_t idx = 0, total = 0;
    ...
    map_unoperable_ch *req = (map_unoperable_ch *)(*data);
    map = global_operating_class;
    ...
    while (map->op_class) {
        ...
        if (cnt != idx) {
            memcpy(req->unopch, ch, cnt);
            memcpy(req->opch, chop, cntop);
            req->num_opch = cntop;
            req->num_unopch = cnt;
            req->opclass = map->op_class;
            // currently per channel, we need to check how to derive it based on regulatory
            req->txpow = ~(pow) + 1;  // 2s complement power
            total++;
            req++;
        }
    ...
    next:
        ...
        cnt = 0, cntop = 0;
        idx = 0;
        memset(ch, MAX_CHANNELS_PER_OPERATING_CLASS, 0x00);
    }
    return total;
}
