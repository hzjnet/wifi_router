...
static void mapapcap(const char *ifname, int argc, char *argv[])
{
	struct iwreq iwr;
	int s, len;
	u_int8_t i, j ;
	...
	iwr.u.data.pointer = (void *) &req;
	iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
	req.cmd = IEEE80211_DBGREQ_SON_MAP_CAP;
	ptr = malloc(32 * sizeof(map_unoperable_ch));
	req.data.mapapcap.length = 32 *sizeof(map_unoperable_ch);
	req.data.mapapcap.data = ptr;

	printf(" %p Length %d \n",req.data.mapapcap.data, req.data.mapapcap.length);

	if (ioctl(s, IEEE80211_IOCTL_DBGREQ, &iwr) < 0) {
		free(ptr);
		errx(1, "IEEE80211_IOCTL_DBGREQ: IEEE80211_DBGREQ_SON_MAP_ASSOC_FRAME failed");
		printf("error in ioctl \n");
		return;
	}

	unop = (map_unoperable_ch *)req.data.mapapcap.data;
	printf("Map AP CAP %x \n",req.data.mapapcap.htcap);
	for (i = 0;i < req.data.mapapcap.opclasscnt; i++)
	{
		printf("Operating class  %d Txpwer %d \n", unop->opclass, unop->txpow);

		for (j = 0; j < unop->num_unopch;j++) {
			printf(" Unoperable channel list %d \n",unop->unopch[j]);
		}
...
static void mapclientcap(const char *ifname, int argc, char *argv[])
{
	struct iwreq iwr;
	int s, len,i;
	struct ieee80211req_athdbg req = {0};
	...
	iwr.u.data.pointer = (void *) &req;
	iwr.u.data.length = (sizeof(struct ieee80211req_athdbg));
	req.cmd = IEEE80211_DBGREQ_SON_MAP_ASSOC_FRAME;
	...
	req.data.mapdata.data = ptr;
	req.data.mapdata.length = 200;
...
#endif
    } else if (streq(cmd, "mapclientcap")) {
	    mapclientcap(ifname, argc, argv);
    } else if (streq(cmd, "mapapcap")) {
	    mapapcap(ifname, argc, argv);
    } else {
        usage();
    }
    return 0;
